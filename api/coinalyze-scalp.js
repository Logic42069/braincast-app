export default async function handler(_req, res) {
  try {
    ...
    */
    ...
    // Determine the trading direction by prioritizing the lower
    // timeframes (5m, 15m, 1h). We sum the number of up and down
    // indicators across these frames.
    const lowerFrames = summary.slice(0, 3);
    let lowerUp = 0;
    let lowerDown = 0;
    lowerFrames.forEach(({ up, down }) => {
      lowerUp += up;
      lowerDown += down;
    });
    const direction = lowerUp >= lowerDown ? 'LONG' : 'SHORT';

    // Determine leverage based on the higher timeframes (4h, 6h, 1d).
    const higherFrames = summary.slice(4);
    let higherUp = 0;
    let higherDown = 0;
    let higherTotal = 0;
